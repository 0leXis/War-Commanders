                        ...
                        if (Upd != ButtonStates.NONE)
                        {
                            if (Upd == ButtonStates.CLICKED)
                            {
                                ...
                                if (MouseControl.Y < CardAppearPos)
                                {
                                    ...
                                    if (Cards[_ChoosedCard].IsTargeted)
                                    {
                                        if (TmpTile != null && PlayerMoney >= Convert.ToInt32(Cards[_ChoosedCard].CostInfo.Text) && TiledMap.CheckTileAllowed(new Point(Tmp[0], Tmp[1]), PlayerZone, Cards[_ChoosedCard].AllowedTiles))//DONE: Убрать костыль TmpTile.NotSelectedFrame != 0, ввести тип Player для поиска правильной стороны
                                        {
                                            //TEST: Юниты
